...
- **남은 위험**(issue 07 결론 참고, 전부 구현 착수 전 항목): 스레드 불일치 시 마샬링
  ...
  포커스별 재구독 로직 미검증.
- **관리자 권한으로 실행 중인 다른 프로세스에는 훅이 안 들어갈 가능성(미검증, 2026-08-15).**
  우리 로더는 일반 권한으로 뜨는데, Windows UIPI(User Interface Privilege Isolation)는
  더 낮은 권한 프로세스가 더 높은 권한(관리자) 프로세스에 코드를 주입/훅 거는 것을 막는다.
  오늘 실사용 검증에 쓴 앱은 전부 일반 권한이라 이 케이스는 한 번도 실측되지 않았다 — 사용자가
  관리자 권한으로 띄운 앱(예: 관리자 권한 cmd, 일부 개발 도구)에서 인디케이터가 그 프로세스의
  상태를 못 따라갈 수 있음.
